...
if __name__ == '__main__':
    ...
    start, end, host, port, outlier_min, outlier_max, test_size, sample_size, sample_rate, neurons, epochs, masking, sample_weight, model_name = (args.start, 
    ...
    # Model building
    build_model(X_train_sampled, y_train_sampled, maxlenlist, neurons, epochs, start, end, sample_rate,
                masking = masking, model_name = model_name, sample_weight=sample_weight)

    # Predict MAE
    pred_mae(X_test_sampled, y_test_sampled, (f'models/{model_name}_{start}_{end}_{epochs}epochs_{maxlen}steps_{sample_rate}srate.h5'), 
            (f"models/scalery_{model_name}_{start}_{end}_{epochs}epochs_{maxlen}steps_{sample_rate}srate.p"), 3, 1)
