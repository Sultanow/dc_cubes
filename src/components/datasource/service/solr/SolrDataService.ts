...
export default class SolrDataService {
    ...
    */
    getLogDataFromSolr = (baseUrl:string, startDateTime: string, endDateTime: string = new Date().toISOString().split('.')[0] + 'Z'): any => {
        //const sortBy = '&sort=timestamp+asc'
        //const query = 'q=*:*&fq=timestamp:[' + startDateTime + ' TO ' + endDateTime + ']' + sortBy;
        const query = '/dc_cubes/query?q=*:*&start=0&rows=30000&sort=id+desc';
