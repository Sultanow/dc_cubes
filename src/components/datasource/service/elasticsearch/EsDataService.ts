...
export default class SolrDataService implements DataSourceService {
    ...
    constructor(esBaseUrl: string, esIndex: string, esForecastIndex: string, esMergedIndex: string) {
        this.esBaseUrl = "http://localhost:9200";
        this.esIndex = esIndex;
        this.esForecastIndex = esForecastIndex;
        this.esMergedIndex = esMergedIndex;
    }
    ...
    */
    getHistorical = (from: string, to: string): any => {
        ...
        const url = this.esBaseUrl + this.esIndex + query;
        return httpClient.get(url);
    };

    getForecast = (from: string, to: string): any => {
        const query = '/query?q=*:*&fq=timestamp:[' + from + ' TO ' + to + ']&sort=timestamp+asc&rows=' + this.resultLimit;
        const url = this.esBaseUrl + this.esForecastIndex + query;
        return httpClient.get(url);
    };
    ...
    getAggregatedLogData = (from: string, to: string, selectedMeasure: string, aggregationType: AggregationType) => {
        ...
        const url = this.esBaseUrl + this.esMergedIndex + "/query"
    ...
    getDistinctTimestamps = (core: string): any => {
        ...
        const url = this.esBaseUrl + core + query;
    ...
    getAggregatedValueForEachTimestamp = (selectedMeasure: string, aggregationType: AggregationType, esIndex: string): any => {
        const query = {
            "query": "*:*",
            "limit": this.resultLimit,
            "facet": {
        ...
        const url = this.esBaseUrl + esIndex + "/query";
    ...
    getMaxValueOfTwoCores = (from: string, to: string, selectedMeasure: string): any => {
        ...
        const url = this.esBaseUrl + this.esIndex + "/query";
        const urlForecast = this.esBaseUrl + this.esForecastIndex + "/query";
