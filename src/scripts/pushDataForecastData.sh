...
importCSV()
{
    ...
    do
        ...
        'http://dc_cubes_solr:8983/solr/dc_cubes_forecast/update/json/docs' \
        --data-binary "$data" > /dev/null # Make curl silent - TODO: debug=true parameter

        # Commit after 1000 entries
        if [ $(($COUNTER % 1000)) == 0 ]; then
            echo "Commiting..."
            curl -s 'http://dc_cubes_solr:8983/solr/dc_cubes_forecast/update?commit=true' > /dev/null
            echo "Commit done."
    ...
    curl 'http://dc_cubes_solr:8983/solr/dc_cubes_forecast/update?commit=true'
    echo -e "\n Done. \n $COUNTER entries imported."
}
