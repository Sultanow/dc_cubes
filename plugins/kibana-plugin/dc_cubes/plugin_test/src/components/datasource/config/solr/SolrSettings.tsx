...
export default class SolrSettings extends Component<SolrSettingsProps, any> {
        ...
        <Tabs defaultActiveKey="home" transition={false} id="noanim-tab-example">
          <Tab eventKey="home" title="Einstellungen">
            <Form>
              ...
              <Form.Group as={Row} controlId="inputSolrBaseUrl">
                ...
                <Col sm="2">{this.props.dataSourceError ? <Badge variant="danger">Offline</Badge> : <Badge variant="success">Online</Badge>}</Col>
              </Form.Group>
          ...
          <Tab eventKey="profile" title="Vorschau">
            ...
            <Table striped bordered hover responsive size="sm" className="previewTable">
              <thead>
                <tr>
                  {this.state.previewData &&
                    Object.keys(this.state.previewData[0]).map((element, index) => {
              ...
              <tbody>
                {this.state.previewData &&
                  this.state.previewData.map((element, index) => {
                    return (
                      <tr key={index}>
                        {Object.values(element).map((elementAttribute, id) => {
  ...
  handleChange = e => {
    ...
    if (name === 'mappingField') {
      this.props.accessChild('customMapping', 'abc' /*eval(value)*/);
    }
    ...
    httpClient
      ...
      .then(data => {
        ...
        strCores.forEach((strCore, index) => {
          dataItems.push(
            <option key={index} value={strCore}>
              {strCore}
            </option>
          );
        });
